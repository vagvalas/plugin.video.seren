...
class RealDebrid:
    ...
    def check_hash(self, hash_value):
        magnet = 'magnet:?xt=urn:btih:{}'.format(hash_value)
        ...
        if "files" in torrent_info:
            torrent_info["files"] = [
                file for file in torrent_info["files"]
                if 'sample' not in file['path'].lower() and source_utils.is_file_ext_valid(file["path"])
            ]
    ...
    def torrent_select_all(self, torrent_id):
        try:
            ...
            g.log("Error selecting files for torrent {}: {}".format(torrent_id, e), "error")
            return None
